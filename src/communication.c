...
#include "../../GuruxDLMS.c-master/development/include/gxkey.h"
#include "../../GuruxDLMS.c-master/development/include/converters.h"
#include "../../GuruxDLMS.c-master/development/include/cosem.h"
...
/**************************plcc added by priya********************************/
#define PLCC_HEADER 64

feature_cfg_t feature_cfg;
ser_port_channel_t ser_port_data[MAX_NO_OF_SERIAL_PORT];
serial_port_diag_t serial_port_diag;
events_data_t event_data;

unsigned char frame[4096];
char event_msg_str[128];

uint8_t g_met_status[MAX_NO_OF_METER_PER_PORT];

int packet_size = 0;

extern Date g_last_met_on_time_info[MAX_NO_OF_METER_PER_PORT];
extern uint8_t g_met_status_flag[MAX_NO_OF_METER_PER_PORT];
    ...
    unsigned char eop)
{
    ...
    int packetLength = 0; // added by priya
    ...
    do
    {
        ...
        if (bytesRead == 0xFFFF)
        {
            ...
            if (errno == EAGAIN)
            {
                // printf("---------------------- readTime %d connection->waitTime %d\n",readTime,connection->waitTime);
        ...
        // printf("BYTES RET : %d SIZE : %d\n", bytesRead, connection->data.size);
        // printf("RESPOSE");
        // for (i = 0; i < connection->data.size; i++)
        // {
        //     printf(" %02x ", connection->data.data[i]);
        // }
        // printf("\n");
        ...
        // Search eop.
        /**************************plcc added by priya********************************/
        // Before it was if (connection->data.size > (5))
        if (strcmp(feature_cfg.meter_inf_type, "PLCC") == 0)
        {
            packet_size = PLCC_HEADER + 5;
        }
        else
        {
            packet_size = 5;
        ...
        {
            if (connection->data.size > packet_size)
            {
        ...
        else
        {
            if (strcmp(feature_cfg.meter_inf_type, "PLCC") != 0)
            {
                if (connection->data.size > 8)
                {
                    packetLength = (connection->data.data[6] << 8) | connection->data.data[7];
                    // Some optical strobes can return extra bytes.
                    if (connection->data.size >= packetLength)
                    {
                        eopFound = 1;
                        break;
                    }
            ...
            {
                // printf("size : %d %d\n", connection->data.size, PLCC_HEADER + 8);
                if (connection->data.size > PLCC_HEADER + 8)
                {
                    // printf("%02x | %02x\n", connection->data.data[70], connection->data.data[71]);
                    packetLength = (connection->data.data[70] << 8) | connection->data.data[71];

                    // printf(" data size : %d wrapper packetLength : %d \n", connection->data.size, packetLength);

                    // Some optical strobes can return extra bytes.
                    if ((connection->data.size >= (packetLength + PLCC_HEADER + 8 + 1)))
                    {
                        // printf("eop found\n");
                        // printf("RESPOSE");
                        // for (i = 0; i < connection->data.size; i++)
...
#if IMX
int rs485_enable(const int fd, int enable)
{
    printf("************ inside rs485 enable ************\n");
    ...
    { // Enable rs485 mode
        ...
        // printf("===> RS485 Enabled successfully\n");
    }
    else
    { // Disable rs485 mode
        rs485conf.flags &= ~(SER_RS485_ENABLED);
    ...
    // printf("===> RS485 Configuration done successfully!!!\n");
    return res;
}
    ...
    connection *connection)
{
...
#else
#if IMX
    rs485_enable(connection->comPort, 1);

#endif
    printf("optical head inside else\n");
    struct termios options;
    memset(&options, 0, sizeof(options));
    options.c_iflag = 0;
    options.c_oflag = 0;
    printf("after optical head inside else");
    if (connection->settings.interfaceType == DLMS_INTERFACE_TYPE_HDLC_WITH_MODE_E)
    {
    ...
    else
    {
        ...
        // Set Baud Rates
        printf(" g_pidx %d\n", g_pidx);
        if (ser_port_data[g_pidx].ser_port_cfg.baud_rate == 600)
        {
            ser_port_data[g_pidx].ser_port_cfg.baud_rate = 0;
        }
        else if (ser_port_data[g_pidx].ser_port_cfg.baud_rate == 1200)
        {
            ser_port_data[g_pidx].ser_port_cfg.baud_rate = 1;
        }
        else if (ser_port_data[g_pidx].ser_port_cfg.baud_rate == 2400)
        {
            ser_port_data[g_pidx].ser_port_cfg.baud_rate = 2;
        }
        else if (ser_port_data[g_pidx].ser_port_cfg.baud_rate == 4800)
        {
            ser_port_data[g_pidx].ser_port_cfg.baud_rate = 3;
        }
        else if (ser_port_data[g_pidx].ser_port_cfg.baud_rate == 9600)
        {
            ser_port_data[g_pidx].ser_port_cfg.baud_rate = 4;
        }
        else if (ser_port_data[g_pidx].ser_port_cfg.baud_rate == 19200)
        {
            ser_port_data[g_pidx].ser_port_cfg.baud_rate = 5;
        }
        else if (ser_port_data[g_pidx].ser_port_cfg.baud_rate == 38400)
        {
            ser_port_data[g_pidx].ser_port_cfg.baud_rate = 6;
        }
        else if (ser_port_data[g_pidx].ser_port_cfg.baud_rate == 115200)
        {
            ser_port_data[g_pidx].ser_port_cfg.baud_rate = 7;
        }

        // ser_port_data[g_pidx].ser_port_cfg.data_bits = 8;
        // ser_port_data[g_pidx].ser_port_cfg.parity = 0; // None
        // ser_port_data[g_pidx].ser_port_cfg.stop_bits = 1;

        printf("ser_port_data[g_pidx].ser_port_cfg.baud_rate %u\nser_port_data[g_pidx].ser_port_cfg.data_bits %u\n ser_port_data[g_pidx].ser_port_cfg.parity  %u\n  ser_port_data[g_pidx].ser_port_cfg.stop_bits %u\n", ser_port_data[g_pidx].ser_port_cfg.baud_rate, ser_port_data[g_pidx].ser_port_cfg.data_bits, ser_port_data[g_pidx].ser_port_cfg.parity, ser_port_data[g_pidx].ser_port_cfg.stop_bits);
        ...
        {
            switch (ser_port_data[g_pidx].ser_port_cfg.parity)
            {
            case 0:
                printf("parity case 0");
                options.c_cflag &= ~PARENB;
                break;
            case 1:
                printf("parity case 1");
                options.c_cflag |= PARENB; // Enable parity
                options.c_cflag |= PARODD;
                break;
            case 2:
                printf("parity case 2");
                options.c_cflag |= PARENB;  // Enable parity
                options.c_cflag &= ~PARODD; //
                break;
            default:
                printf("parity case def");
                options.c_cflag &= ~PARENB;
                break;
            }

            switch (ser_port_data[g_pidx].ser_port_cfg.stop_bits)
            {
            case 1:
                printf("stopbits case 1");
                options.c_cflag &= ~CSTOPB;
                break;
            case 2:
                printf("stopbits case 2");
                options.c_cflag |= CSTOPB;
                break;
            default:
                printf("stopbits case defa");
                options.c_cflag &= ~CSTOPB;
                break;
            }

            switch (ser_port_data[g_pidx].ser_port_cfg.data_bits)
            {
            case 7:
                printf("databit case 7");
                options.c_cflag &= ~CSIZE; // Clear the data size bits
                options.c_cflag |= CS7;
                break;
            case 8:
                printf("databit case 8");
                options.c_cflag &= ~CSIZE; // Clear all bits that set the data size
                options.c_cflag |= CS8;
                break;
            default:
                printf("databit case 9");
                options.c_cflag &= ~CSIZE; // Clear all bits that set the data size
                options.c_cflag |= CS8;
                break;
            }
            switch (ser_port_data[g_pidx].ser_port_cfg.baud_rate)
            {
            case 0:
...
#endif
    printf("interfaceType: %d\n", connection->settings.interfaceType);
    if (connection->settings.interfaceType == DLMS_INTERFACE_TYPE_HDLC_WITH_MODE_E)
    {
    ...
    const char *port)
{
    ...
    printf("inside com_open 2------- 17/7\n");
    if (connection->comPort == INVALID_HANDLE_VALUE)
    {
        printf("inside com_open 3------- 17/7\n");
        ret = GetLastError();
        ...
        sprintf(event_data.eventmsg, "Main_port Failed to open serial port: \"%s\"", buff);
    ...
    if (connection->comPort == -1) // if open is unsuccessful.
    {
        ret = errno;
        printf("Failed to open serial port: %s\n", port);
        printf("inside com_open 5------- 17/7\n");
        return DLMS_ERROR_TYPE_COMMUNICATION_ERROR | ret;
    }
    if (!isatty(connection->comPort))
    {
        printf("inside com_open 6------- 17/7\n");
        ret = errno;
        printf("Failed to Open port %s. This is not a serial port.\n", port);
        return DLMS_ERROR_TYPE_COMMUNICATION_ERROR | ret;
    }
    printf("inside com_open 7------- 17/7\n");
#endif
...
int sendData(connection *connection, gxByteBuffer *data)
{
...
#else
        // rithika 02/12/2025
        //  dbg_log(INFORM, "[Meter_%d]:[%-25s] : [Number of bytes : %d] : Tx : ", curr_nc_idx, "send_Data", data->size);
        for (int i = 0; i < data->size; ++i)
        {
            printf("%02X ", data->data[i]);
        }
    ...
    {
        // rithika 02/12/2025
        //    dbg_log(INFORM, "[Meter_%d]:[%-25s] : [Number of bytes : %d] : Tx : %s\n", curr_nc_idx, "sendData", data->size, (const char *)data->data);
...
int readData(connection *connection, gxByteBuffer *data, int *index)
{
    ...
    if (connection->comPort != INVALID_HANDLE_VALUE)
    {
        ...
        {
            /**************************plcc added by priya********************************/
            serial_port_diag.invaild_resp_cnt++;
            /**************************plcc added by priya********************************/
            return ret;
        }
        else
        {
            /**************************plcc added by priya********************************/
            // else case added
            serial_port_diag.vaild_resp_cnt++;
            /**************************plcc added by priya********************************/
        }
    }
    else
    ...
    if (connection->trace > GX_TRACE_LEVEL_INFO)
    {
        ...
        {
            /**************************plcc added by priya********************************/
            memcpy((frame + *index), (connection->data.data + *index), (connection->data.size - *index));

            // rithika 13nov2025
            print_data(curr_nc_idx, connection->data.data + *index, connection->data.size - *index);
            // dbg_log(INFORM, "[Meter_%d]:[%-25s] : [Number of bytes : %d] : Rx : %s\n",curr_nc_idx, "readData",  connection->data.size, hex);
            /**************************plcc added by priya********************************/
        }
        else
        {
    ...
    gxReplyData *reply)
{
    ...
    /**************************plcc added by priya********************************/
    gxByteBuffer send_data;
    gxByteBuffer plcc_data;
    memset(&send_data, 0, sizeof(gxByteBuffer));
    ...
    if (strcmp(feature_cfg.meter_inf_type, "PLCC") == 0)
    {
        ...
        memset(&plcc_data, 0, sizeof(gxByteBuffer));

        plcc_data.size = plcc_packetization(data->data, data->size, temp_data);
        plcc_data.data = temp_data;
    }
    else
    {
        send_data.size = data->size;
    ...
    if (strcmp(feature_cfg.meter_inf_type, "PLCC") == 0)
    {
        ...
        {
            hex = bb_toHexString(&plcc_data);
            // printf("\nTX:\t%s\n", hex);
            // printf("\n");
            dbg_log(INFORM, "[Meter_%d]:[%-25s] : [Number of bytes : %d] : Tx : %s\n", curr_nc_idx, "readDLMSPacket", plcc_data.size, hex);
            free(hex);
        }

        if ((ret = sendData(connection, &plcc_data)) != 0)
        {
            return ret;
        }
    }
    ...
    { // added by rithika
        if (connection->trace == GX_TRACE_LEVEL_VERBOSE)
        {
            ...
            // printf("\nrithika TX:\t%s\n", hex);
            dbg_log(INFORM, "[Meter_%d]:[%-25s] : [Number of bytes : %d] : Tx : %s\n", curr_nc_idx, "readDLMSPacket", send_data.size, hex);
            free(hex);
        }
    ...
    do
    {
        ...
        if ((ret = readData(connection, &connection->data, &index)) != 0)
        {
            ...
            //     printf("%02X ", connection->data.data[i]);
            // }
            printf("ret = %d disc_ret %d\n",ret, disc_ret);

            // rithika 10Dec2025
            if (ret == DLMS_ERROR_CODE_RECEIVE_FAILED && disc_ret == 1)
            {
                dbg_log(WARNING, "[Meter_%d]:[%-25s] : Data receive failed. \n", curr_nc_idx, "readDLMSPacket");

                break;
            }
            else if (ret != DLMS_ERROR_CODE_RECEIVE_FAILED || pos == 3)
            {

                /**************************plcc added by priya********************************/
                if (pos == 3)
                {
                    // print_colored_message(RED,PRINT,"Meter_%d commn flag status : %d\n",curr_nc_idx,g_met_status_flag[curr_nc_idx]);

                    memset(&g_last_met_on_time_info[curr_nc_idx], 0, sizeof(Date));
                    memcpy(&g_last_met_on_time_info[curr_nc_idx], &meter_date_time[curr_nc_idx], sizeof(Date));

                    g_met_status[curr_nc_idx] = 0;
                    if (strcmp(feature_cfg.meter_inf_type, "PLCC") == 0)
                    {
                        plcc_response_parsing(connection->data.data, connection->data.size);
                    }
                    serial_port_diag.no_resp_cnt++;
                    if (g_met_status_flag[curr_nc_idx] != 0)
                    {
                        // print_colored_message(RED,PRINT,"Meter_%d Not Communicating Time : %s\n",curr_nc_idx,time_format_conversion(time(NULL)));
                        g_met_status_flag[curr_nc_idx] = 0;
                        memset(event_msg_str, 0, sizeof(event_msg_str));

                        sprintf(event_data.eventmsg, "%d:%d Meter is not communicating",
                                (curr_nc_idx + 1), g_pidx);
                    ...
                    if (strcmp(feature_cfg.meter_inf_type, "PLCC") == 0)
                    {
                        plcc_modem_info.plcc_meter_map[curr_nc_idx].signal_quality = 0;
                        // plcc_modem_info.plcc_meter_map[curr_nc_idx].rs_status = 0;
                        set_plcc_modem_info_to_redis();
                    }
                    if (OD_CMD_RECEIVED == 1)
                    {
                        // commented rithika 29Nov2025
                        //  update_cmd_status(od_cmd, "In Progress: Meter didnt respond for the command");
                    }
                }
                /****************************************************************************/
                break;
            ...
            dbg_log(WARNING, "[Meter_%d]:[%-25s] : Data receive failed. Try to resend : %d\n", curr_nc_idx, "readDLMSPacket", pos);

            if ((ret = sendData(connection, &send_data)) != 0)
            {
                break;
            }
        }
        else
        {
            ...
            if ((connection->settings.interfaceType != DLMS_INTERFACE_TYPE_HDLC) && ((strcmp(feature_cfg.meter_inf_type, "PLCC") == 0)))
            {
                memcpy(connection->data.data, frame, connection->data.size);
                memset(frame, 0, connection->data.size);

                int temp_size = wrapper_depacketization(connection->data.data, connection->data.size, frame);
                connection->data.size = temp_size;

                memset(connection->data.data, 0, temp_size);
                memcpy(connection->data.data, frame, temp_size);

                // printf("ORGINAL DATA SIZE : %d temp size : %d capacity : %d\n", connection->data.size, temp_size, connection->data.capacity);
            }
    ...
    if ((strcmp(feature_cfg.meter_inf_type, "PLCC") == 0))
    {
        ...
        {
            plcc_modem_info.plcc_meter_map[curr_nc_idx].signal_quality = connection->data.data[32];
        }

        // printf("SIGNAL QUALITY : %d\n", plcc_modem_info.plcc_meter_map[curr_nc_idx].signal_quality);
        ...
        if (connection->settings.interfaceType == DLMS_INTERFACE_TYPE_HDLC)
        {
            ...
            int temp_size = plcc_depacketization(connection->data.data, connection->data.size, frame);
            connection->data.size = temp_size;
    ...
    serial_port_diag.recv_resp_cnt++;
    /****************************************************************************/
    ...
    gxReplyData *reply)
{
    ...
    for (pos = 0; pos != messages->size; ++pos)
    {
        ...
        {
            printf("!!!!!!!!!!!com_readdatablock failed ret %d !!!!!!!!!!!!!!\n", ret);
            return ret;
        }
        else
        {
            printf("************************************* inside meters communication status update\n");
            /**************************plcc added by priya********************************/
            dbg_log(INFORM, "[Meter_%d]:[%-25s] :calling meter_commn_status with arg as 1 \n", curr_nc_idx, "com_readDataBlock");

            meter_commn_status(curr_nc_idx, 1);

            print_colored_message(RED, PRINT, "Meter_%dcommn flag status : %d\n", curr_nc_idx, g_met_status_flag[curr_nc_idx]);

            if (g_met_status_flag[curr_nc_idx] != 1)
            {

                // print_colored_message(RED,PRINT,"Meter_%d Communicating Time : %s\n",curr_nc_idx,time_format_conversion(time(NULL)));
                g_met_status_flag[curr_nc_idx] = 1;
                // preethi 08Nov2k23
                memset(event_msg_str, 0, sizeof(event_msg_str));

                sprintf(event_data.eventmsg, "%d:%d Meter is communicating",
                        (curr_nc_idx + 1), g_pidx);
            ...
            // plcc_modem_info.plcc_meter_map[curr_nc_idx].rs_status = 1;
            g_met_status[curr_nc_idx] = 1;
            /****************************************************************************/
        }
    ...
    printf("!!!!!!!!!!!com_readdatablock ret %d !!!!!!!!!!!!!!\n", ret);
    return ret;
}
    ...
    connection *connection)
{
    ...
    dbg_log(WARNING, "[Meter_%d]:[%-25s] : Sending DISC\n", curr_nc_idx, "com_disconnect");
    ...
    {
        dbg_log(WARNING, "[Meter_%d]:[%-25s] : DISC Failed\n", curr_nc_idx, "com_disconnect");
    }
    else
    {
        dbg_log(WARNING, "[Meter_%d]:[%-25s] : DISC Success\n", curr_nc_idx, "com_disconnect");
    }
    ...
    connection *connection)
{
    ...
    if (!connection->settings.server)
    {
        ...
        /**************************plcc added by priya********************************/
#if 0 // some meter will not suppport release because of that I commeneted for now.priya.
    ...
    if (invocationCounter != NULL && connection->settings.cipher.security != DLMS_SECURITY_NONE)
    {
            ...
            (ret = cl_parseUAResponse(&connection->settings, &reply.data)) != 0)
        {
            ...
            if (connection->trace > GX_TRACE_LEVEL_OFF)
            {
                ...
                dbg_log(REPORT, "[Meter_%d]:[%-25s] : SNRMRequest failed %s\n", curr_nc_idx, "com_updateInvocationCounter", hlp_getErrorMessage(ret));
            }
            return ret;
        }
        mes_clear(&messages);
        ...
        connection->settings.connected = 1;
            ...
            (ret = cl_parseAAREResponse(&connection->settings, &reply.data)) != 0)
        {
            ...
            if (ret == DLMS_ERROR_CODE_APPLICATION_CONTEXT_NAME_NOT_SUPPORTED)
            {
                if (connection->trace > GX_TRACE_LEVEL_OFF)
                {
                    ...
                    dbg_log(REPORT, "[Meter_%d]:[%-25s] : Use Logical Name referencing is wrong. Change it!\n", curr_nc_idx, "com_updateInvocationCounter");
                }
                return ret;
            }
            if (connection->trace > GX_TRACE_LEVEL_OFF)
            {
                ...
                dbg_log(REPORT, "[Meter_%d]:[%-25s] : AARQRequest failed %s\n", curr_nc_idx, "com_updateInvocationCounter", hlp_getErrorMessage(ret));
            }
            return ret;
        }
        mes_clear(&messages);
        ...
        else
        {
            ...
            con_initializeBuffers(connection, 500 + connection->settings.maxPduSize);
        }
        gxData d;
    ...
    connection *connection)
{
        ...
        (ret = cl_parseUAResponse(&connection->settings, &reply.data)) != 0)
    {
        ...
        {
            printf("============ SNRMRequest failed %s ============\n", hlp_getErrorMessage(ret));
            dbg_log(REPORT, "[Meter_%d]:[%-25s] : SNRMRequest failed %s\n", curr_nc_idx, "com_initializeConnection", hlp_getErrorMessage(ret));
        }
        return ret;
    }
    mes_clear(&messages);
    ...
    {
        printf("inisde aarq if block in com_initializeConnection\n");
        if ((ret = cl_aarqRequest(&connection->settings, &messages)) != 0 ||
            (ret = com_readDataBlock(connection, &messages, &reply)) != 0 ||
            (ret = cl_parseAAREResponse(&connection->settings, &reply.data)) != 0)
        {
            ...
            if (ret == DLMS_ERROR_CODE_APPLICATION_CONTEXT_NAME_NOT_SUPPORTED)
            {
                ...
                {
                    printf("Use Logical Name referencing is wrong. Change it!\r\n");
                    dbg_log(REPORT, "[Meter_%d]:[%-25s] : Use Logical Name referencing is wrong. Change it!\n", curr_nc_idx, "com_initializeConnection");
                }
                return ret;
            }
            if (connection->trace > GX_TRACE_LEVEL_OFF)
        ...
        else
        {
            ...
            /**************************plcc modified by priya********************************/
            // Before con_initializeBuffers(connection, 50 + 1024);
            // con_initializeBuffers(connection, 50 + 1024);

            con_initializeBuffers(connection, 500 + 1024);
            /*******************************************************************************/
        }
    ...
    printf("end of com_initializeConnection ret %d\n", ret);
                     ...
                     unsigned char attributeOrdinal, int ret)
{
    ...
    dbg_log(WARNING, "[%-25s] : %s %s %s:%d %s \n", "com_reportError", description, obj_typeToString2(object->objectType), ln, attributeOrdinal, hlp_getErrorMessage(ret));
}
...
int com_getAssociationView(connection *connection, const char *outputFile)
{
    ...
    printf("******************************* %d *************************\n", connection->settings.objects.size);
    ...
    struct tm *end)
{
    ...
    if ((ret = cl_readRowsByRange(&connection->settings, object, start, end, &data)) != 0 ||
        ...
        (ret = cl_updateValue(&connection->settings, (gxObject *)object, 2, &reply.dataValue)) != 0)
    {
        dbg_log(INFORM, "[Meter_%d]:[%-25s] : ReadObject failed %s\n", curr_nc_idx, "com_readRowsByRange", hlp_getErrorMessage(ret));
    }
    mes_clear(&data);
    reply_clear(&reply);
    return ret;
